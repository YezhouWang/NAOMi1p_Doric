...
dims_um = min(dims, [vol_sz(1), vol_sz(2), vol_sz(3)] ./ dimsSS);          % dims set at dimsSS um per space
dims_um = floor(dims_um);        % <-- MISSING STEP IN NAOMi
dims    = dims_um * vres;                                                  
dims = floor(max(dims, 1));% Calculate dimensions of the sampled array
