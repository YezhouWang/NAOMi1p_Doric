...
vol_sz_um    = vol_params.vol_sz;                 % microns
vol_sz_px    = round(vol_sz_um * vol_params.vres);% voxels
vol_depth_px = round(vol_params.vol_depth * vol_params.vres);

vol_sz_px(3) = min(vol_sz_px(3), vol_params.vol_sz(3) * vol_params.vres);
% ===== Z safety margin (voxels) =====
max_rad_um = neur_params.avg_rad * neur_params.exts(2); % worst-case soma radius
z_margin_px = ceil(max_rad_um * vol_params.vres);

assert(vol_sz_px(3) > 2*z_margin_px, ...
    'Z volume too shallow for neuron soma size');

nz_ves = size(neur_ves_trunc,3);

z_start = min(vol_depth_px + 1, nz_ves);
z_end   = min(vol_depth_px + vol_sz_px(3), nz_ves);

% --- Crop mesh grids to match idx_good Z-range --- deleted by 2025/12/31
% mesh_x = mesh_x(:,:,z_start:z_end);
% mesh_y = mesh_y(:,:,z_start:z_end);
% mesh_z = mesh_z(:,:,z_start:z_end);

if z_end <= z_start
    error('Invalid Z range: check vol_depth and vasc_sz');
end
fprintf('[sampleDenseNeurons] neur_ves Z=%d, vol_depth_px=%d, vol_sz_px(3)=%d\n', ...
        size(neur_ves_trunc,3), vol_depth_px, vol_sz_px(3));

% Extract valid Z range from vasculature
idx_good = neur_ves_trunc(:,:,z_start:z_end);

% Determine common Z size
nz_idx  = size(idx_good,3);
nz_mesh = size(mesh_z,3);
nz_use  = min(nz_idx, nz_mesh);

% Crop BOTH to the same Z extent
idx_good = idx_good(:,:,1:nz_use);
mesh_x   = mesh_x(:,:,1:nz_use);
mesh_y   = mesh_y(:,:,1:nz_use);
mesh_z   = mesh_z(:,:,1:nz_use);

fprintf('[sampleDenseNeurons] Using Z slices: %d (idx=%d, mesh=%d)\n', ...
        nz_use, nz_idx, nz_mesh);

assert(isequal(size(idx_good), size(mesh_x)), ...
    'Size mismatch: idx_good and mesh grids must match');
...
while (sum(idx_good(:))>1)&&(size(Vcell,3)-isempty(Vcell)<vol_params.N_neur)% WHILE the requested number of neurons is not reached AND there is still room to place neurons....
    ...
    % ===== Enforce Z-boundary safety =====
    z_margin_um = z_margin_px / vol_params.vres;
    new_pt(3) = min( ...
        max(new_pt(3), z_margin_um), ...
        vol_sz_um(3) - z_margin_um ...
        );
...
fprintf('neur_ves size (px): %d %d %d\n', size(neur_ves_trunc));
fprintf('vol depth (px): %d\n', vol_depth_px);
fprintf('vol z size (px): %d\n', vol_sz_px(3));
