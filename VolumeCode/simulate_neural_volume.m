...
% ---- DENDRITE DISABLE SAFETY ----
if isfield(dend_params,'flag') && ~dend_params.flag
    vol_params.N_den = 0;
end
...
% ---- FORCE CONSISTENT INTEGER VOXEL GEOMETRY ----
vol_params.vol_sz    = round(vol_params.vol_sz / vol_params.vres) * vol_params.vres;
vol_params.vol_depth = round(vol_params.vol_depth / vol_params.vres) * vol_params.vres;

assert(all(mod(vol_params.vol_sz ./ vol_params.vres, 1) == 0), ...
    'vol_sz / vres must be integer');
assert(mod(vol_params.vol_depth / vol_params.vres, 1) == 0, ...
    'vol_depth / vres must be integer');
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
vol_params.vasc_sz = round(vol_params.vasc_sz / vol_params.vres) * vol_params.vres; % patch for NAOMi; make sure integers
assert(all(mod(vol_params.vasc_sz ./ vol_params.vres, 1) == 0), ...
    'vasc_sz / vres must be integer');
...
if(vasc_params.flag)
  ...
  neur_ves = false( ...
    round(vol_params.vol_sz(1) * vol_params.vres), ...
    round(vol_params.vol_sz(2) * vol_params.vres), ...
    round((vol_params.vol_sz(3) + vol_params.vol_depth) * vol_params.vres));
  neur_ves_all = false(round(vol_params.vres * vol_params.vasc_sz));                %        ... then set up a fully empty volume
end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
fprintf('N_neur = %d, N_den = %d\n', vol_params.N_neur, vol_params.N_den);

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% Add apical dendrites from bottom of volume to top

if dend_params.flag
    [neur_num, neur_num_AD, dend_params] = growApicalDendrites(vol_params, dend_params, ...
        neur_num, cellVolumeAD, gp_nuc, gp_soma); % Grow out apical dendrites
    clear cellVolumeAD
else
    neur_num_AD = neur_num;
end
