...
vol_sz_um = vol_params.vol_sz;
vres      = vol_params.vres;

vol_sz_px = round(vol_sz_um * vres);   % ALWAYS integer voxel grid
...
neur_soma = zeros(vol_sz_px, 'uint16');                                 % Initialize the array that stores where all the different somas are
neur_vol  = zeros(vol_sz_px, 'single');                                 % Initialize the full neural voume array
...
vol_depth_px = round(vol_params.vol_depth * vres);

z_start = vol_depth_px + 1;
z_end   = min(vol_depth_px + vol_sz_px(3), size(neur_ves,3));

taken_pts = taken_pts(:,:,z_start:z_end);

% Safety crop
assert(all(vol_sz_px > 0), 'Invalid voxel volume size');
fprintf('[generateNeuralVolume] voxel size: %d %d %d\n', vol_sz_px);
...
for kk = 1:vol_params.N_neur
    ...
    idx_pos = max(1, round(vres * neur_locs(kk, :)));
    idxX = max(1,idx_pos(1)-vres*max_ext):min(idx_pos(1)+vres*max_ext, vol_sz_px(1)); % modified um-based to piex-based unit
    idxY = max(1,idx_pos(2)-vres*max_ext):min(idx_pos(2)+vres*max_ext, vol_sz_px(2));
    idxZ = max(1,idx_pos(3)-vres*max_ext):min(idx_pos(3)+vres*max_ext, vol_sz_px(3));

    % --- HARD SAFETY GUARD ---
    idxX = round(idxX);
    idxY = round(idxY);
    idxZ = round(idxZ);
    idxX = idxX(idxX >= 1 & idxX <= vol_sz_px(1));% ensures absolute safety even if a neuron is near a boundary 
    idxY = idxY(idxY >= 1 & idxY <= vol_sz_px(2));
    idxZ = idxZ(idxZ >= 1 & idxZ <= vol_sz_px(3));
    if isempty(idxX) || isempty(idxY) || isempty(idxZ)
    continue
    end
    assert(all(idxX >= 1) && all(idxY >= 1) && all(idxZ >= 1)) % for debug
...
%                  mesh_y(idx_to_test),mesh_z(idx_to_test)]);                % Find the points inside the K^th nucleus to remove (no fluorescence)
    ...
    iY = iY+idxY(1)-1;
    iZ = iZ+idxZ(1)-1;
    neur_idx2              = sub2ind(vol_sz_px,iX,iY,iZ);
...
%     neur_idx               = false(vol_sz_px);                          % Set up an array indexing the points inside the neuron
