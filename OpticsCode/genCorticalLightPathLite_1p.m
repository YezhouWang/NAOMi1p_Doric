...
stride = psf_samp * vres * ss;
imax = min(imax, floor((size(phzA,1) - N) / stride) + 1);
jmax = min(jmax, floor((size(phzA,2) - N) / stride) + 1);
...
% this block is commented to skip axial interpolation entirely, because DeepWonder does not use this information
% z_src = linspace(0, psf_params.taillength, numel(psfTMz)); 
% z_q   = 0:1/vres:psf_params.taillength;
% psfTMz = interp1(z_src, psfTMz, z_q, 'linear', 'extrap');
% psfBMz = interp1(z_src, psfBMz, z_q, 'linear', 'extrap');
