...
% modified by YW. last update: 1/21/2026
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% Parse inputs
DEBUG = false;   % <<< TEMPORARY
% for debug
dbstop if error
warning('on','all')
disp('scan_volume_1p: start')
...
if isfield(scan_params,'start_Nt_override') && scan_params.start_Nt_override
    start_Nt = 0;
else
    start_Nt = 100;
end    
...
% scale_ratio = vol_params.vres / pixel_size; % YW_debug, set trtio as 1 to make sure output movie is 512x512
scale_ratio = 1; % YW_debug, set trtio as 1 to make sure output movie is 512x512
...
if (~isfield(scan_params,'zoffset'))||(isempty(scan_params.zoffset))
    z_base = floor((N3 - Np3)/2) + 1;
else
    z_base = floor((N3 - Np3)/2) + 1 + scan_params.zoffset;
end

% hard safety clamp
z_base = max(1, min(z_base, N3 - Np3 + 1));
...
% ---- 2D miniscope PSF (no FFT volume convolution) ----
% freq_psf = []; % YW_debug: freq_psf should not be [] if we run spatial Convolution for the 2D PSF
freq_psf = PSF;      % psf2 is spatial PSF
...
for ll = 1:size(soma_min,1)
    ...
    f0vol = array_SubSubTest_matlab(f0vol, somaVol{ll,1}, somaVol{ll,2}, soma_min(ll)); % Iteratively add in each neuron's soma activity
end
for ll = 1:size(soma_min,1)
    f0vol_wo_bg = array_SubSubTest_matlab(f0vol_wo_bg,somaVol{ll,1},somaVol{ll,2},soma_min(ll));      % Iteratively add in each neuron's soma activity
end
...
for ll = 1:size(neur_vol.gp_nuc,1)
    ...
    elseif numel(neur_vol.gp_nuc{ll,2}) == numel(neur_vol.gp_nuc{ll,1})
        f0vol = array_SubSubTest_matlab(f0vol,(neur_vol.gp_nuc{ll,1}), ...
...
for ll = 1:size(dend_min,1)                                                % For large volumes, mex functions decrease runtimes a lot
    f0vol = array_SubSubTest_matlab(f0vol,dendVol{ll,1},dendVol{ll,2},dend_min(ll));      % Iteratively add in each neuron's soma activity
end
for ll = 1:size(bg_min,1)
    if(~isempty(axonVol{ll,1}))
        f0vol= array_SubModTest_matlab(f0vol, (axonVol{ll,1}),axonVol{ll,2},bg_min(ll)); % Iteratively add in each background component's activity
    end
end
if(isfield(scan_params,'nuc_label'))&&(scan_params.nuc_label>=1)
    ...
    for ll = 1:size(nucVol,1)
      f0vol = array_SubSubTest_matlab(f0vol,nucVol{ll,1},nucVol{ll,2},nuc_min(ll));       % Iteratively add in each neuron's soma activity
    end
end
fprintf('base initialized.\n')

if DEBUG
    assert(isfloat(f0vol), 'f0vol not float');
    assert(~any(isnan(f0vol(:))), 'NaN in f0vol');
    assert(z_loc >= 1);
    assert(z_loc+Np3-1 <= size(f0vol,3));
    assert(all(size(f0vol(:,:,z_loc:(z_loc+Np3-1))) > 0));
    keyboard;   % <<< STOP HERE
end

% img_w0 = single_scan(f0vol(:,:,z_loc:(z_loc+Np3-1)), size(PSF), freq_psf, scan_avg, true); % YW_debug: if true, 
% Frequency-domain convolution (FFT-based) will be used, but for 1-p microscope we don't use it
img_w0 = single_scan(f0vol(:,:,z_loc:(z_loc+Np3-1)), size(PSF), freq_psf, scan_avg); 
...
if ~isstruct(PSF)
    for kk = start_Nt + 1 : Nt  % Iterate over time steps
        ...
        if vessel_mod.flag
            ...
            % vessel_img = ones(size(WMPvol_w_bg(:, :, z_loc:(z_loc+Np3-1)))); % YW_debug: replace this line when we disable vessel modulation
            vessel_img = ones(N1, N2, Np3, 'single'); % YW_debug
        end
        ...
        for ll = 1:size(soma_act,1)                                        % For large volumes, mex functions decrease runtimes a lot
            if(soma_act(ll,kk)>0)&&(~isempty(somaVol{ll,2}))
                WMPvol_w_bg = array_SubSubTest_matlab(WMPvol_w_bg,somaVol{ll,1},...
        ...
        if(isfield(scan_params,'nuc_label'))&&(scan_params.nuc_label>=1)
            for ll = 1:size(neur_vol.gp_nuc,1)
                if(nuc_act(ll,kk)>0)&&(~isempty(nucVol{ll,2}))
                    WMPvol_w_bg = array_SubSubTest_matlab(WMPvol_w_bg,nucVol{ll,1},...
                                        nucVol{ll,2},nuc_act(ll,kk));    % Iteratively add in each neuron's soma activity
                    WMPvol_w_bg_pure = array_SubSubTest_matlab(WMPvol_w_bg_pure,nucVol{ll,1},...
                                        nucVol{ll,2},nuc_act(ll,kk));    % Iteratively add in each neuron's soma activity                
                end            
            end
        end
...
%         % dend
        for ll = 1:size(dend_act,1)                                        % For large volumes, mex functions decrease runtimes a lot
            if (dend_act(ll,kk)>0)&&(~isempty(dendVol{ll,1}))
                WMPvol_w_bg = array_SubSubTest_matlab(WMPvol_w_bg,dendVol{ll,1},...
                                           dendVol{ll,2},dend_act(ll,kk)); % Iteratively add in each neuron's soma activity
                WMPvol_w_bg_pure = array_SubSubTest_matlab(WMPvol_w_bg_pure,dendVol{ll,1},...
                                           dendVol{ll,2},dend_act(ll,kk)); % Iteratively add in each neuron's soma activity
            end
        end
        ...
        for ll = 1:size(bg_act,1)
            if(~isempty(axonVol{ll,1}) && bg_act(ll,kk)>0)
                WMPvol_w_bg_pure= array_SubModTest_matlab(WMPvol_w_bg_pure, axonVol{ll,1}, ...
        ...
        clean_img_w_bg = (sigscale/(2*sfrac^2))*single_scan((WMPvol_w_bg(:,:,...
                  z_loc:(z_loc+Np3-1))+f0vol(:,:,z_loc:(z_loc+Np3-1))) .* vessel_img,...
                                     size(PSF), freq_psf, scan_avg); % Scan a single frame, normalization for pixel size and scanning code
