...
%% config
% FOV_sz = 282;% FOV, um repeated
% nt = 500;  % frames repeat
% fn = 5; % frame rate
% pavg = 1; %mW per mm^2 repeat


%% parser
% vol_params.vol_sz(1) = FOV_sz; repeated
% vol_params.vol_sz(2) = FOV_sz; repeated
% spike_opts.nt = nt + 100; % repeat
% spike_opts.dt = 1 / fn;
% wdm_params.pavg = pavg; repeat

%% vessel modulation
vessel_mod.flag = false; % true for enable vessel modulation.
vessel_mod.frate = frate; % frame rate
...
vessel_mod.seed = 10; 
%% output path
output_dir = sprintf('D:/a_job/Doric/results/synthetic', ...
...
%% generate volume
tic
vasc_params.flag = false;
dend_params.flag = false; % change to false to disable dendrite growth 
vol_params.N_den = 0; % 0 = no dendrites
...
fun_time_trace_generation(vol_out, nt, frate, output_dir)
...
% scan_params.start_Nt_override = true;  % enable this line if in debug mode
